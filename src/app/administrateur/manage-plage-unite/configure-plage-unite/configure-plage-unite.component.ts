...
export class ConfigurePlageUniteComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.plageUniteService.getAllPlagesByUnite(this.uniteId)
      .subscribe(
        (data: any) => {
          this.plages = data;
          if (!this.plages.length) {
            ...
            for (let i = 0; i < this.plages.length; i++) {
              setTimeout(function () {
                console.log("init:    " + (i));
                baseContext.initSelect2Types(i);
                baseContext.initSelect2Jours(i);
              }, 20);
            }
          }
        }
  ...
  onSubmit() {
    ...
    if ((!this.seanceService.getAllSeances())) {
      this.busy = this.plageUniteService.editPlages(this.plages, this.uniteId)
        .subscribe(data => {
            console.log(data);
            console.log("success");
            const baseContext = this;
  ...
  private initSelect2Jours(index: number) {
    ...
    selectJour.select2();
    const baseContext = this;
    selectJour.val(baseContext.plages[index].jour)
      .trigger('change');
  ...
  private initSelect2Types(index: number) {
    ...
    selectType.select2();
    const baseContext = this;
    selectType.val(baseContext.plages[index].type_id)
      .trigger('change');
